...
class API360:
    ...
    def get_all_contacts(self, file=False):
        """
        Get all contacts of the organisation
        :return:
        """
        response = requests.get(f"{self.url}/external_contacts?perPage={self.per_page}", headers=self.headers)
        if not response.ok:
            print(f"{response}:{response.text}")
            return {}
        ...
        for i in range(2, response.json()["pages"] + 1):
            successful = False
            while not successful:
                response = requests.get(f"{self.url}/external_contacts?perPage={self.per_page}&page={i}", headers=self.headers)
                try:
        ...
        # Get all users of the organisation and write info about them in file:
        if file:
            API360.save_file("contacts_output", contacts)
        return contacts
    ...
    def post_create_contact(self, contact_info: List):
        """
        Creating the new contact with the provided dict
        :param users_info: list of the dicts with the new contact info
        :return: displays success or error message
        """
    ...
    def delete_contact_by_id(self, contact_id):
        response = requests.delete(f"{self.url}/external_contacts/{contact_id}", headers=self.headers)

        try:
            if response.ok:
               print(f'Contact with id: {contact_id} was deleted successfully')
        except requests.exceptions.JSONDecodeError:
            print(f"error decoding response: {response}")
    ...
    def wipe_all_contacts(self):
        for id in list(x.get('id') for x in self.get_all_contacts()):
